...
project: IProject; showDetail:null|number; setShowDetail:(id:null|number)=>void;
}> = ({
project:{id, name, image_path, category, deployed_url, description, github_url, key_techs}, showDetail, setShowDetail,
  ...
  return (
    <div>
      ...
      <Image width="300" height="150" layout="responsive" quality="75" src={image_path} alt={name} className="cursor-pointer" onClick={()=>setShowDetail(id)}/>
      ...
      {showDetail === id && (
        <div className="absolute top-0 left-0 z-10 grid w-full h-auto p-2 text-black bg-gray-100 rounded-lg md:p-10 md:grid-cols-2 gap-x-12 dark:text-white dark:bg-dark-100">
          <motion.div variants={stagger} initial="initial" animate="animate">
            ...
            <motion.div variants={fadeInUp} className="border-4 border-gray-900 dark:border-gray-100">
              <Image width="300" height="150" layout="responsive" quality="75" src={image_path} alt={name} />
            </motion.div>
          ...
          <motion.h2 variants={fadeInUp} className="mb-3 text-xl font-medium md:text-2xl">{name}</motion.h2>
            <motion.h3 variants={fadeInUp} className="mb-3 text-medium">{description}</motion.h3>
